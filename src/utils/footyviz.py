...
def draw_frame(df, t, dpi=100, fps=20, display_num=False, display_time=False, show_players=True,
    ...
    if show_players and (dfFrame is not None):
...
def add_voronoi_to_fig(fig, ax, dfFrame, hide=True):
    polygons = {}
...
def get_frame(df, t, fps=20):
    frameid = int(t*fps)+1
    if frameid not in df.index.tolist():
        return None
    else:
        dfFrame = df.loc[frameid].set_index('player')
